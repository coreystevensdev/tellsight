...
describe('POST /unsubscribe/digest/[token]', () => {
  ...
  it('returns 502 UPSTREAM_UNAVAILABLE when the request arrives with an already-aborted signal', async () => {
    ...
    const request = new NextRequest('http://localhost/unsubscribe/digest/sometoken', { method: 'POST' });
    ...
    expect(warnSpy).toHaveBeenCalledWith('[unsubscribe-bare-route] upstream unreachable', expect.anything());
  });
});

...
describe('POST /api/mute/alert-rule/[token]', () => {
  it('passes through a successful upstream response', async () => {
    ...
    const res = await POST(new NextRequest('http://localhost/api/mute/alert-rule/sometoken', { method: 'POST' }), { params });
  ...
  it('returns 502 UPSTREAM_UNAVAILABLE when fetch rejects', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith({ err: expect.any(Error) }, '[mute-route] upstream unreachable');
  });
  ...
  it('returns the upstream status with UPSTREAM_INVALID_RESPONSE when the body is not JSON', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith({ err: expect.any(SyntaxError) }, '[mute-route] upstream returned non-JSON body');
  });
  ...
  it('forwards to the expected upstream URL', async () => {
    ...
    await POST(new NextRequest('http://localhost/api/mute/alert-rule/sometoken', { method: 'POST' }), { params });
  ...
  it('returns 502 UPSTREAM_UNAVAILABLE when the request arrives with an already-aborted signal', async () => {
    ...
    const request = new NextRequest('http://localhost/api/mute/alert-rule/sometoken', { method: 'POST' });
    ...
    expect(warnSpy).toHaveBeenCalledWith({ err: expect.anything() }, '[mute-route] upstream unreachable');
  });
});

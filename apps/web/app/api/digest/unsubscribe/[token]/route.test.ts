...
describe('POST /api/digest/unsubscribe/[token]', () => {
  ...
  it('forwards to the expected upstream URL', async () => {
    ...
    await POST(new NextRequest('http://localhost/api/digest/unsubscribe/sometoken', { method: 'POST' }), { params });
  ...
  it('returns 502 UPSTREAM_UNAVAILABLE when the request arrives with an already-aborted signal', async () => {
    ...
    const request = new NextRequest('http://localhost/api/digest/unsubscribe/sometoken', { method: 'POST' });

...
describe('GET /mute/alert-rule/[token]', () => {
  it('redirects to the confirm page without calling fetch', () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch');

    const res = GET(new NextRequest('http://localhost/mute/alert-rule/sometoken'));

    expect(res.status).toBe(307);
    expect(res.headers.get('location')).toBe('http://localhost/mute/alert-rule/sometoken/confirm');
    expect(fetchSpy).not.toHaveBeenCalled();
  });
});

describe('POST /mute/alert-rule/[token]', () => {
  it('passes through a successful upstream response', async () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce({
      status: 200,
      json: () => Promise.resolve({ data: { muted: true } }),
    } as Response);

    const res = await POST(new NextRequest('http://localhost/mute/alert-rule/sometoken', { method: 'POST' }), { params });

    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ data: { muted: true } });
  ...
  it('returns 502 UPSTREAM_UNAVAILABLE when fetch rejects', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith('[mute-bare-route] upstream unreachable', expect.any(Error));
  });
  ...
  it('returns the upstream status with UPSTREAM_INVALID_RESPONSE when the body is not JSON', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith('[mute-bare-route] upstream returned non-JSON body', expect.any(SyntaxError));
  });
  ...
  it('forwards to the expected upstream URL', async () => {
    ...
    await POST(new NextRequest('http://localhost/mute/alert-rule/sometoken', { method: 'POST' }), { params });

    expect(fetchSpy).toHaveBeenCalledWith(
      expect.stringContaining('http://api:3001/alerts/mute/sometoken'),
  ...
  it('returns 502 UPSTREAM_UNAVAILABLE when the request arrives with an already-aborted signal', async () => {
    ...
    const request = new NextRequest('http://localhost/mute/alert-rule/sometoken', { method: 'POST' });
    ...
    expect(warnSpy).toHaveBeenCalledWith('[mute-bare-route] upstream unreachable', expect.anything());
  });
});

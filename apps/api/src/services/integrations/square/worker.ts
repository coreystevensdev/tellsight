...
export const SYNC_QUEUE_NAME = 'square-sync';
...
export async function enqueueSyncJob(connectionId: number, trigger: SyncTrigger): Promise<void> {
  ...
  logger.info({ connectionId, trigger }, 'Enqueued Square sync job');
}
...
export function initSyncWorker(): Worker {
  ...
  worker.on('error', (err) => {
    logger.error({ err }, 'Square sync worker error');
  });

  logger.info({ concurrency: WORKER_CONCURRENCY }, 'Square sync worker started');
  return worker;
}
...
export async function shutdownWorker(): Promise<void> {
  ...
  if (worker) {
    logger.info({}, 'Closing Square sync worker');

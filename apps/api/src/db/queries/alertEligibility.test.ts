...
// SQL-shape test rig, same precedent as digestEligibility.test.ts and
// alertRules.test.ts (Story 10.1, deferred-work.md item 3): a lazy postgres-js
// tag that never opens a socket, only `.toSQL()` gets called against it.
...
const { findEligibleOrgs, buildEligibilityQuery } = await import('./alertEligibility.js');
...
describe('buildEligibilityQuery: SQL shape', () => {
  it('emits all required predicates in the WHERE clause', () => {
    ...
    expect(sql).toMatch(/"alert_rules"\."enabled"\s*=\s*\$/);
    expect(sql).toMatch(/"alert_rules"\."deleted_at"\s+is\s+null/i);
  });

  it('binds the literal values "active" and "pro"', () => {
    const { params } = buildEligibilityQuery(inertDb as never).toSQL();
    expect(params).toContain('active');
    expect(params).toContain('pro');
  });

  it('excludes rules whose mute_until is still in the future', () => {
    const { sql } = buildEligibilityQuery(inertDb as never).toSQL();
    expect(sql).toMatch(/"alert_rules"\."mute_until"\s+is\s+null/i);

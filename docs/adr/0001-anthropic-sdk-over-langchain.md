...
- The privacy edge. The "evidence must cite an allowed stat ID, else drop" rule in `validateProposalCandidate` enforces the no-raw-data-leak boundary at the output. A generic output parser would not scope that for us.

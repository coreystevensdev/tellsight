...
- **About one summary in thirty-five carries no insight that is both non-obvious and actionable.** The product commits to at least one per analysis, and until recently nothing measured it: the eval graded whether claims were true and whether the stats were covered, and a summary can score full marks on both by restating every figure and suggesting nothing. A judge for it now runs in the harness. Measured at 3 misses in 105 generations on the deployed v2.0 prompt, 2.9%, 95% CI 1.0% to 8.1%. Getting there took two corrections worth naming. The rate was first reported as 1 in 35 off a single 35-sample run, which was the optimistic end of a wide interval rather than the estimate; a second run put the pooled v1.9 figure at 6 in 95. That larger sample then showed the misses were not spread evenly, they concentrated on the cash-crunch fixture at 15.6% against 2.5% elsewhere, and the cause was a prompt bug: the cash-flow rule offered "worth reviewing with your accountant" as its worked example, the model copied it verbatim, and a referral naming nothing to review is not a next step. It attached itself to the most prominent figure on the page, the runway, which the owner already knows. v2.0 requires the referral to name what to examine. Cash-crunch went to 4.6% on 65 samples, Fisher p = 0.088, while the one fixture with no cash-flow stat, where that rule cannot fire, stayed at zero in both versions. The gate is a pooled miss rate against a 15% ceiling rather than a per-sample floor, because at the old default of nine samples a single miss reads as 11% and a zero-miss gate fails a healthy run about a tenth of the time. The default is thirty samples now: that moves the false-fail rate to 3.8% and raises the chance of catching a v1.7-sized regression from 77% to 95%. The harness runs offline rather than in CI, so this is a number to re-measure rather than a gate that holds it.

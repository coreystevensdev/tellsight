...
- **About one summary in thirty-five carries no insight that is both non-obvious and actionable.** The product commits to at least one per analysis, and until recently nothing measured it: the eval graded whether claims were true and whether the stats were covered, and a summary can score full marks on both by restating every figure and suggesting nothing. A judge for it now runs in the harness. Measured at 1 miss in 35 generations on the deployed v1.9 prompt, sampled 12 per fixture so no one fixture carries the number, which matches v1.8's 1 in 36 and is down from 5 in 18 before a prompt change that binds the observation to its next step. The model had been producing both halves reliably but in separate sentences, and the requirement asks for one insight carrying both. The gate is a pooled miss rate against a 15% ceiling, not a per-sample floor. The floor was the literal reading of the requirement and useless in practice: at the default nine samples a single miss is 11%, so a zero-miss gate fails a healthy run about a quarter of the time, and a gate that cries wolf gets ignored. At that sample size it catches a regression the size of v1.7's and nothing finer. The harness runs offline rather than in CI, so this is a number to re-measure rather than a gate that holds it.

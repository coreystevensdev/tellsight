...
- **Roughly one summary in thirty carries no insight that is both non-obvious and actionable.** The product commits to at least one per analysis, and until recently nothing measured it: the eval graded whether claims were true and whether the stats were covered, and a summary can score full marks on both by restating every figure and suggesting nothing. A judge for it now runs in the harness, gated per sample rather than on an average, because an average lets one analysis ship with none. Measured at about 2 in 70 sampled generations after a prompt change that binds the observation to its next step, down from 5 in 18 before it: the model had been producing both halves reliably but in separate sentences, and the requirement asks for one insight carrying both. The remaining misses are real, not a threshold artifact, and the harness runs offline rather than in CI, so this is a number to re-measure rather than a gate that holds it.

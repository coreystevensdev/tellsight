...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-4%2C492-brightgreen.svg" alt="4,492 tests">
...
**Live:** [tellsight.coreystevens.dev](https://tellsight.coreystevens.dev), no signup needed, the dashboard loads with seed data and a real AI summary. Signing up gives you your own organization, seeded with the same twelve months so there is something to interpret straight away, and Stripe runs in test mode, so `4242 4242 4242 4242` with any future expiry reaches Pro and the full-length interpretation without a real charge. **Deploy:** AWS EC2 t3.micro + RDS PostgreSQL 18 + Redis 7 (Docker Compose, co-located). GitHub Actions OIDC deploys via SSM SendCommand; no SSH key stored. See [infra/README.md](infra/README.md) for the Terraform runbook.

Most analytics tools show numbers. This one explains what they mean, and delivers the interpretation to your inbox every week. Connect QuickBooks, Shopify or Square, or upload a CSV (the four data sources supported today), get charts, then a plain-English explanation of what the trends actually mean for your business. Sign in with Google or email and password. Multi-tenant Postgres with row-level security, SSE streaming for AI summaries, BullMQ three-queue digest pipeline, Stripe billing. The AI only ever sees computed statistics, never raw rows. 4,492 automated tests (4,415 Vitest plus 77 Playwright E2E), with the curation pipeline's financial math the most heavily covered.
...
Upload a CSV or connect QuickBooks, Shopify or Square directly via OAuth (the four supported data sources today; Stripe and bank-feed connectors are planned but not yet built). A CSV keeps whatever column names the exporting tool wrote, so Square, QuickBooks and Shopify exports are matched under their own headers rather than being renamed by hand first. The dashboard instantly visualizes revenue trends, expense breakdowns, and category comparisons. Then AI reads the computed statistics (not your raw data) and explains what's happening in plain English: which costs are rising faster than revenue, where seasonal patterns suggest opportunities, what anomalies deserve attention. Connected users skip the CSV export entirely. Pro users get that interpretation delivered as a weekly email digest, with week-over-week context built in, so the analysis arrives without having to remember to log in.
...
An offline eval harness grades the summaries that come out: three labeled financial fixtures (healthy-growth, cash-crunch, seasonal-anomaly) run through the full pipeline and are judged for faithfulness (no invented figures), completeness (covers the stats that matter), and legal posture (analytics framing, not financial advice). Faithfulness and completeness use LLM judges via the shared provider; legal posture is a deterministic string scanner with 31 tests, run via `pnpm test:eval`, separate from CI. Run the full harness with `pnpm eval`. Measured across roughly 70 sampled generations on 2026-09-08, rather than one run, because the per-run spread is wider than the differences worth reporting: faithfulness lands between 0.96 and 0.99 aggregate against a floor of 0.85, completeness at 1.00, and legal posture fails about one sample in thirty-five. A further 155 samples on the deployed v2.0 prompt on 2026-09-10 held all three: faithfulness 0.99 aggregate with a 0.83 worst case, completeness 1.00, legal posture clean on every sample. A fourth judge grades the non-obvious-and-actionable insight the product commits to (see Known limitations for the rate). Legal posture flagged one of nine cash-crunch samples for the literal phrase "you need to," and that was read at the time as a genuine slip past the prompt. It was not. The sentence was "the gap between what you're earning and what you need to break even," which names a figure rather than telling anyone to do anything, and a substring match cannot separate the two. The matcher now exempts that phrase after a relative pronoun and the eval calls the same matcher production does, rather than rebuilding the regex from the shared list and drifting from it. That check is deterministic and runs offline here at QA time; the live SSE stream now runs the same check at request time too (see Known Limitations for why flagging, not blocking, is the ceiling for a streamed response). A second track, `pnpm eval:qa`, grades the Q&A tool's lookup-vs-interpretation bias with an LLM judge, backed by a deterministic numeric-figure guard covered by `interpretation-guard.test.ts` (12 tests); last measured run: 1.00 aggregate accuracy across all 8 fixtures, floor 0.85. The eval:qa track doesn't run in CI either.
...
- **Stripe runs in test mode, deliberately.** Gating the AI interpretation behind a paywall nobody reviewing this will pay is self-defeating, and a demo that charged a real card would need business registration, tax handling and refund obligations to be real too. So `4242 4242 4242 4242` reaches Pro. Everything either side of the card is the production path: the same hosted checkout, a real `checkout.session.completed` with its signature verified, and the same free-to-Pro transition a paying customer takes. `config.ts` refuses a test key in production unless the deployment says it means it, and the boot check reports which mode it is actually in at `/api/health` rather than trusting the key prefix.

- **The deployed demo sets no connector credentials, so all three are off.** QuickBooks, Shopify and Square are built against their sandbox environments, and each gates off entirely unless its credentials are present. The live instance sets none of them, so CSV upload is the only source a visitor can reach, the connector routes answer `501 INTEGRATION_NOT_CONFIGURED`, and the boot log says so once per provider. The code paths are the production ones: real webhook signature verification, real OAuth with state and HMAC checks, real token refresh against the provider's live token endpoint. Only the credentials differ, and `SQUARE_ENVIRONMENT` / `QUICKBOOKS_ENVIRONMENT` switch it with no code change. A public demo that charged a visitor's card would need business registration, tax handling and refund obligations to be real too, and a connector pointed at a live seller account would put actual business revenue on a single free-tier instance for a benefit no visitor could see, since signed-out visitors are served synthetic seed data regardless.
...
- **Four data sources today: QuickBooks, Shopify and Square OAuth, plus CSV upload.** There are no connectors for Stripe, bank feeds, or other accounting platforms yet. If your data isn't already in one of those three, a CSV export is the only way in. Each connector reads its own OAuth credentials from config, so a deployment that has not set them shows CSV only. Square reads sales and refunds across every location on the account; it does not read inventory the way the Shopify connector does.
...
- **The live AI summary can't be blocked from delivering directive language, only flagged after the fact.** The eval harness has flagged real ones, though the first hit it reported turned out to be a descriptive clause the substring matcher could not tell from a directive. The agent-proposal pipeline can reject this at the schema boundary because a proposal is a complete object before it's ever used; a summary streams token by token as it generates, so by the time the full text is assembled and checked, it has already reached the browser. `streamHandler.ts` now runs the same `hasDirectiveLanguage` check every other summary validator uses (unknown stat refs, unknown citations, unmatched figures) and logs + tracks an analytics event when it fires, but "flag it" is the ceiling for a streamed response; true blocking would mean buffering the whole generation before sending any of it, trading away the streaming UX to close a rare edge case.
- **About one summary in twenty-two carries no insight that is both non-obvious and actionable.** The product commits to at least one per analysis, and until recently nothing measured it: the eval graded whether claims were true and whether the stats were covered, and a summary can score full marks on both by restating every figure and suggesting nothing. A judge for it now runs in the harness. Measured at 7 misses in 155 generations on the deployed v2.0 prompt, 4.5%, 95% CI 2.2% to 9.0%. That figure has been revised down twice and up once, and the revisions are the most useful thing in this section. It was first reported as 1 in 35 off a single 35-sample run, then corrected to 1 in 16 when a second run pooled to 6 in 95, then briefly reported as 1 in 35 again off 105 samples, and settled at 1 in 22 only after a pre-committed 50-sample arm came back worse than the three before it, which had read 8%, 0% and 5%. A single run of this eval does not pin a rate, and the failure mode is always the same: an early arm lands low and the number gets quoted as though the interval around it were narrow. The concentration was real, though. v1.9 missed 15.6% on the cash-crunch fixture against 2.5% elsewhere, traced to the cash-flow rule offering "worth reviewing with your accountant" as its worked example, which the model copied verbatim; a referral naming nothing to review is not a next step, and it attached itself to the runway, the figure the owner already knows. v2.0 requires the referral to name what to examine and takes cash-crunch to 6.1% on 115 samples, Fisher p = 0.068, so the gain is real-looking and not proven. The one fixture carrying no cash-flow stat, where that rule cannot fire, stayed at zero across both versions. The gate is a pooled miss rate against a 15% ceiling rather than a per-sample floor, and the default sample size is thirty, which puts the false-fail rate at 3.8% and the chance of catching a v1.7-sized regression at 95%. The harness runs offline rather than in CI, so this is a number to re-measure rather than a gate that holds it.

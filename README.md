...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-4%2C255-brightgreen.svg" alt="4,255 tests">
...
Most analytics tools show numbers. This one explains what they mean, and delivers the interpretation to your inbox every week. Connect QuickBooks or Shopify, or upload a CSV (the three data sources supported today), get charts, then a plain-English explanation of what the trends actually mean for your business. Sign in with Google or email and password. Multi-tenant Postgres with row-level security, SSE streaming for AI summaries, BullMQ three-queue digest pipeline, Stripe billing. The AI only ever sees computed statistics, never raw rows. 4,255 automated tests (4,178 Vitest plus 77 Playwright E2E), with the curation pipeline's financial math the most heavily covered.
...
- **Three data sources today: QuickBooks OAuth, Shopify OAuth, and CSV upload.** There are no connectors for Stripe, bank feeds, or other accounting platforms yet. If your data isn't already in QuickBooks or Shopify, a CSV export is the only way in. Both connectors read their own OAuth credentials from config, so a deployment that has not set them shows CSV only.

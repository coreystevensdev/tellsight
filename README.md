...
- **About one summary in sixteen carries no insight that is both non-obvious and actionable, and the misses cluster on businesses under cash pressure.** The product commits to at least one per analysis, and until recently nothing measured it: the eval graded whether claims were true and whether the stats were covered, and a summary can score full marks on both by restating every figure and suggesting nothing. A judge for it now runs in the harness. Measured across 95 generations on the deployed v1.9 prompt in two runs, 6 misses, 6.3%, 95% CI 2.9% to 13.1%. The first run of 35 returned 1 miss and the second of 60 returned 5; those are consistent with one underlying rate rather than a regression (Fisher p = 0.41), which is the case for quoting the pooled figure and the interval rather than either run alone. An earlier draft of this section said one in thirty-five, from the first run; that is the optimistic end of the interval, not the estimate. All of it is down from 5 in 18 on v1.7, before a prompt change that binds the observation to its next step, since the model had been producing both halves reliably but in separate sentences and the requirement asks for one insight carrying both. The larger sample also showed something the smaller one could not: 4 of the 6 misses are the cash-crunch fixture, 20% against 2.5% for the other two (Fisher p = 0.038). A business running out of money is the case where the next step is most obvious and hardest to make non-obvious, and it is the one worth attacking next. The gate is a pooled miss rate against a 15% ceiling, not a per-sample floor. The floor was the literal reading of the requirement and useless in practice: at the default nine samples a single miss is 11%, so a zero-miss gate fails a healthy run often enough to get ignored. The ceiling sits just above the measured upper bound, so it tolerates normal variance while still catching a regression the size of v1.7's. The harness runs offline rather than in CI, so this is a number to re-measure rather than a gate that holds it.
